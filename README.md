...
This project uses Vite. If you need runtime configuration, add a `.env` or `.env.local` file at the project root and prefix client variables with `VITE_` (for example `VITE_RPC_URL` or `VITE_CONTRACT_ACCOUNT_ADDRESS`). Example variables you might add:

```
VITE_RPC_URL=https://rpc.example
VITE_CHAIN_ID=1221
VITE_ACCOUNT_ADDRESS=0x...
```
...
This repository does not include a license by default. Add a `LICENSE` file if you intend to publish under a specific license.
...
VITE_RPC_URL=https://rpc.example
VITE_CHAIN_ID=1337
VITE_CONTRACT_ACCOUNT_ADDRESS=0x...
```
